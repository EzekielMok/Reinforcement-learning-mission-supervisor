...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def sample(self, n):
        b_idx, b_memory, ISWeights = np.empty((n,), dtype=np.int64), np.empty((n, self.tree.data[0].size)), np.empty((n, 1))
...
class HybPrioritizedReplayDueling:
    def __init__(
            self,
            n_actions,#动作空间
            n_features,#动作空间的特征
            learning_rate=0.005,#学习律
            reward_decay=0.9,#收益衰减
            e_greedy=0.95,# 贪婪策略
    ...
    def _build_net(self):
        def build_layers(s, c_names, n_l1, w_initializer, b_initializer,trainable):
            ...
            if self.dueling:# 网络选择，此处是一个逻辑变量
                # Dueling DQN
    ...
    def plot_cost(self):
        ...
        plt.savefig('I://dqn_analyse//dqn_data//RLcost.jpg')
        plt.show()
        file = open('I://dqn_analyse//dqn_data//' + 'cost' + '.txt', 'a+')
